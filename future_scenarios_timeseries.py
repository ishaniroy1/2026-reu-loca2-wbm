...
for model_var, livneh_var in var_mapping.items():
    ...
    with xr.open_dataset(LIVNEH_REF) as obs_ds:
        obs_mask = regionmask.mask_3D_geopandas(northeast_gdf, obs_ds['lon'], obs_ds['lat']).any(dim='region')
        ...
        ax.plot(obs_annual['time'].dt.year, obs_annual.values,
                color='black', linewidth=2.5, label='Livneh Observations (1980-2014)')
    ...
    for ssp_key, ssp_info in scenarios.items():
        ...
        for folder in ssp_folders:
            ...
            try:
                with xr.open_mfdataset(model_pattern, combine='by_coords') as model_ds:
                    # slice time
                    model_trimmed = model_ds[model_var].sel(time=slice('2015-01-01', '2100-12-31'))

                    # generate spatial mask for model's resolution layout
                    m_mask = regionmask.mask_3D_geopandas(northeast_gdf, model_trimmed['lon'], model_trimmed['lat']).any(dim='region')

                    spatial_mean = model_trimmed.where(m_mask).mean(dim=['lat', 'lon'])
                ...
                print(f"Skipping future processing for {model_name} ({ssp_key}): {e}")
                continue
        ...
        if all_model_series:
            ...
            ax.plot(years, ensemble_mean.values,
            ...
            ensemble_min = ensemble_annual.min(dim='model')
            ensemble_max = ensemble_annual.max(dim='model')

            ax.fill_between(years, ensemble_min.values.flatten(), ensemble_max.values.flatten(), color=ssp_info['color'], alpha=0.10)
    ...
    ax.text(2027, y_text_pos, 'Early-Century\n(2015-2040)', ha='center', va='top', fontsize=9, style='italic', color='navy')
    ax.text(2055, y_text_pos, 'Mid-Century\n(2041-2070)', ha='center', va='top', fontsize=9, style='italic', color='darkgoldenrod')
    ax.text(2085, y_text_pos, 'Late-Century\n(2071-2100)', ha='center', va='top', fontsize=9, style='italic', color='darkred')

    # figure labels
    unit_label = "Precipitation (mm/month)" if model_var == 'precip' else "Temperature (°C)"
    ax.set_ylabel(unit_label, fontsize=12)
    ax.set_xlabel("Year", fontsize=12)
    ax.set_title(f"Northeast Region Future Projections (2015-2100): {model_var}", fontsize=14, fontweight='bold')

    ax.grid(True, linestyle=':', alpha=0.5)
    ax.legend(loc='upper left', framealpha=0.9, fontsize=9)
    plt.tight_layout()
    
    output_plot = os.path.join(OUTPUT_DIR, f"{model_var}_future_timeseries.png")
